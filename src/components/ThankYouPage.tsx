...
const ThankYouPage = ({
  ...
  useEffect(() => {
    ...
    const fetchLatestOrder = async () => {
      const {
        data,
        error
      } = await supabase.from('orders').select('id, customer_name, customer_phone').order('created_at', {
        ascending: false
      }).limit(1).single();
      if (!error && data) {
  ...
  return <div className="container max-w-2xl mx-auto py-16 px-4 text-center">
      <div className="mb-8">
